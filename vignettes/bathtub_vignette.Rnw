...
\usepackage{graphicx}
\usepackage[left=1 in, top= 0.75 in, right=1 in,head = 0.75 in , foot=0.75 in]{geometry}
...
\title{Demonstration of the textreg Package}
...
\subsection{Installing the textreg package}
First install the \verb|Rcpp| and \verb|tm| packages.
...
\subsection{Getting ready to regress}
...
\subsection{Obtaining the Summary}
...
<<plot_results, echo=TRUE, fig.height=4>>=
plot( rs )
@
...
corpus = build.corpus( bathtub, mth.lab, banwords, verbosity=0 )
...
@
Note here we pre-build the corpus with the \verb|build.corpus| command, and then pass it to the several \verb|textreg| commands.  This can help if the corpus is large and/or being loaded from a file since we then do not need to copy over the R text (if loaded in R) or file into the computer memory used by C++.

Once we have our lists, we can merge them to see overlap quite easily via the \verb|make.list.table| command.
...
<<plot_different_models, echo=TRUE, fig.height=4, fig.width=5>>=
...
rs.lit = textreg( bathtub, mth.lab.lit, C = 4, gap=1, min.support = 1, verbosity=0 )
rs.lit
rs.lit$labeling
@
...
\subsection{Finding Where Phrases Appear}
...
\subsection{Independent Search Methods}
...
\subsection{Finding Phrases' Contexts}
...
\subsection{Relationships between phrases}
...
\section{Cleaning Text and Stemming}
